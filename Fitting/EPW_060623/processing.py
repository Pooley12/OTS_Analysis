...
if __name__ == '__main__':
    ...
    for t in range(0, len(Raw_times), 1):
        ...
        if os.path.exists(MCMC_stats_file):
            ...
            MCMC_stats_df['Mean Ne (1/cc)'].append(round(df.loc['NE', 'Mean'], 2))
            MCMC_stats_df['Std Ne (1/cc)'].append(round(df.loc['NE', 'Std'], 2))
            MCMC_stats_df['Mean Ne_range (1/cc)'].append(round(df.loc['NE_RANGE', 'Mean'], 2))
            MCMC_stats_df['Std Ne_range (1/cc)'].append(round(df.loc['NE_RANGE', 'Std'], 2))
        else:
