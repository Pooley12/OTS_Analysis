...
class Fitting:
    ...
    def generate_fit(self, params):  
        ...
        model_x, model_y = self.OTS_model.run_fitting(params, self.EXPLORE.param_names, use_zbar_eos=True, use_range=True)
...
if __name__ == '__main__':
    ...
    for t in range(0, len(Raw_times), 1):
        ...
        Names, Units, Mins, Maxs, Logbool = Exploration().param_names, Exploration().param_units, Exploration().param_mins, Exploration().param_maxs, Exploration().param_logbool
        ...
        if Run_CMAES:
            ...
            if Save_info:
                save_cmaes_file(CMAES_loc, Exploration().param_names, Best_inits, Exploration().param_units)
            ...
            try:
                Best_inits = extract_cmaes_file(CMAES_loc, Exploration().param_names)
                Best_norm_inits = Run.normalize(Best_inits)
                print('Extracted best initial values from CMAES fit')
            except:
                Best_norm_inits = Exploration().norm_param_inits
                Best_inits = Exploration().param_inits
